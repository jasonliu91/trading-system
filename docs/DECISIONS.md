...
## D028: 前端 UX 改进——移动端导航、骨架屏、WebSocket 重连
**日期**: 2026-02-13
**决策**: 添加移动端汉堡菜单导航、骨架屏加载状态、错误重试按钮、WebSocket 指数退避重连（最多10次，1s-30s）、页面可见性 API 暂停重连、K线图成交量柱
**原因**:
- 原导航栏在移动端溢出，需要折叠式汉堡菜单
- 数据加载时无视觉反馈，用户体验差
- WebSocket 断连后无重连机制，实时数据丢失
- K线图缺少成交量信息，分析不完整
**关键约束**:
- WebSocket 重连最大30秒间隔，超过10次停止自动重连
- 骨架屏仅在首次加载时显示，后续刷新保留旧数据

## D027: 后端可靠性改进——重试、验证、错误处理
**日期**: 2026-02-13
**决策**: Binance API 增加指数退避重试（最多3次）；orchestrator 分阶段捕获异常并记录计时和连续失败次数；Market Mind 保存前执行 JSON schema 验证；健康检查端点增加数据库连通性和数据新鲜度检测；绩效端点从真实交易数据计算指标
**原因**:
- Binance API 偶发超时会导致整个分析周期失败，重试可恢复临时故障
- orchestrator 缺乏分阶段错误隔离，单阶段失败导致整个周期无输出
- Market Mind 手工编辑可能产生非法结构，需要前置校验
- 健康检查需要真实验证组件状态，不能只返回静态 "ok"
- 绩效数据硬编码无法反映实际交易表现
**关键约束**:
- Binance 重试仅对网络错误生效，HTTP 4xx 错误不重试
- Market Mind 验证返回警告但不阻止保存，保持向后兼容
- 数据新鲜度阈值为6小时

## D026: 后端单元测试与代码文档化
**日期**: 2026-02-13
**决策**: 为核心模块（risk engine, decision engine, paper engine, market mind）添加 51 个单元测试；所有核心模块函数添加中文 docstring；Trade 表添加 decision_id 外键和复合索引
**原因**:
- 零测试覆盖导致重构和修改缺乏安全网
- 缺少 docstring 增加新开发者理解成本
- Trade 与 Decision 缺少关联，无法追溯交易对应的决策依据
**关键约束**:
- 测试使用 SQLite 内存数据库，每个用例独立
- docstring 使用中文，与项目文档语言保持一致
- 外键 `decision_id` 为 nullable，兼容历史数据
